...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["budget"])) {
    ...
    $updateQuery = "UPDATE project_details SET final_budget = '$budget' WHERE project_name = '$projectName'";
    $updateResult = mysqli_query($conn, $updateQuery);

    if ($updateResult) {
        echo "<script>alert('Budget Updated Successfully!'); window.location.href = 'mentorProjects.php'; </script>";
    } else {
        echo "<script>alert('Budget Update Failed!'); window.location.href = 'mentorProjects.php';</script>";
    }
}

if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["stageName"])) {
    $stageName = $_POST["stageName"];
    $statusName = $_POST["statusName"];
    $projectName = $_POST["projectName"];

    $statusValue = ($statusName == '1') ? 1 : 0;

    if ($stageName == 'stage1') {
        $updateQuery = "UPDATE project_details SET stage_1 = '$statusValue' WHERE project_name = '$projectName'";
        $updateResult = mysqli_query($conn, $updateQuery);

        if ($updateResult) {
            echo "<script>alert('Stage 1 Status Updated Successfully!'); window.location.href = 'mentorProjects.php'; </script>";
        } else {
            echo "<script>alert('Stage 1 Status Update Failed!'); window.location.href = 'mentorProjects.php';</script>";
        }
    } else if ($stageName == 'stage2') {
        $updateQuery = "UPDATE project_details SET stage_2 = '$statusValue' WHERE project_name = '$projectName'";
        $updateResult = mysqli_query($conn, $updateQuery);

        if ($updateResult) {
            echo "<script>alert('Stage 2 Status Updated Successfully!'); window.location.href = 'mentorProjects.php'; </script>";
        } else {
            echo "<script>alert('Stage 2 Status Update Failed!'); window.location.href = 'mentorProjects.php';</script>";
        }
    } else if ($stageName == 'stage3') {
        $updateQuery = "UPDATE project_details SET stage_3 = '$statusValue' WHERE project_name = '$projectName'";
        $updateResult = mysqli_query($conn, $updateQuery);

        if ($updateResult) {
            echo "<script>alert('Stage 3 Status Updated Successfully!'); window.location.href = 'mentorProjects.php'; </script>";
        } else {
            echo "<script>alert('Stage 3 Status Update Failed!'); window.location.href = 'mentorProjects.php';</script>";
        }
    }
...
<body>
    ...
    <div class="py-20 px-16 sm:ml-64">
        ...
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg mt-14">
            <p class="text-3xl text-black p-5">Update Project Budget</p>
            <div class="flex items-center justify-center h-fit mb-4 rounded bg-gray-50 ">
                    <form class="space-y-4 md:space-y-6 p-2" action="" method="post">
                    <input type="hidden">
                        <div>
                            <label for="projectSelect" class="block mb-2 text-sm font-medium text-gray-900">Select Project</label>
                            <select name="projectName" id="projectSelect" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full md:w-96 p-2.5" required="">
                                <?php
                                $projectQuery = "SELECT project_name FROM project_details";
                                $projectResult = mysqli_query($conn, $projectQuery);

                                while ($projectRow = mysqli_fetch_assoc($projectResult)) {
                                    echo "<option value=\"{$projectRow['project_name']}\">{$projectRow['project_name']}</option>";
                                }
                                ?>
                            </select>
                        </div>
                        ...
                        <div>
                            <label for="budget" class="block mb-2 text-sm font-medium text-gray-900">Update Budget</label>
                            <input type="text" name="budget" id="budget" placeholder="50000" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full md:w-96 p-2.5" required="">
                        </div>

                        <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Update Final Budget</button>
                    </form>
            </div>
        </div>
        
        <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg mt-14">
            <p class="text-3xl text-black p-5">Update Project Status</p>
            <div class="flex items-center justify-center h-fit mb-4 rounded bg-gray-50 ">
            <form class="space-y-4 md:space-y-6 p-2" action="" method="post">
                        <div>
                            <label for="projectSelect" class="block mb-2 text-sm font-medium text-gray-900">Select Project</label>
                        ...
                        <div>
                            <label for="stageSelect" class="block mb-2 text-sm font-medium text-gray-900">Select Stage</label>
                            <select name="stageName" id="stageSelect" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full md:w-96 p-2.5" required="">
                        ...
                        <div>
                            <label for="statusSelect" class="block mb-2 text-sm font-medium text-gray-900">Select Status</label>
                            <select name="statusName" id="statusSelect" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full md:w-96 p-2.5" required="">
                        ...
                        <button type="submit" class="w-full text-white bg-blue-700 hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Update Stage</button>
                    </form>
            </div>
        </div>
